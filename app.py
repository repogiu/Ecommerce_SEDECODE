...
# Ruta para manejar la adición de productos mediante un formulario en la página de administración
@app.route('/admin/products/send', methods=['POST'])
def admin_products_send():
    # Recibe los datos del formulario y los guarda en variables
    _name = request.form['txtNombre']  # Nombre del producto
    _precio = request.form['txtPrecio']  # Precio del producto
    _stock = request.form['txtStock']  # Stock del producto
    _imagen_url = request.form['txtImagen']  # URL de la imagen del producto

    # Imprimir los valores en la consola para verificar que se están recibiendo correctamente
    print(_name)
    print(_precio)
    print(_stock)
    print(_imagen_url)
    ...
    try:
        # Ejecuta una consulta SQL para insertar el nuevo producto en la base de datos
        cur.execute(
            "INSERT INTO dim_product (product_name, product_price, product_stock, product_url, updated_at) VALUES ("
            "%s, %s, %s, %s, NOW())",
            (_name, _precio, _stock, _imagen_url)  # Pasar los valores como parámetros
        )
        # Confirmar los cambios en la base de datos
        conn.commit()
    except Exception as e:
        # Imprimir un mensaje de error en caso de que ocurra un problema durante la inserción
        print(f"Error al guardar en la base de datos: {e}")
    finally:
        # Cerrar el cursor y la conexión a la base de datos, independientemente de si hubo un error o no
        cur.close()
        conn.close()

    # Redirigir al usuario de vuelta a la página de administración de productos después de guardar
    return redirect('/admin/products')
...
# Ruta para eliminar productos basada en el ID del producto recibido en la URL
...
def delete_product(product_id):
    ...
    try:
        # Ejecuta la consulta SQL para eliminar el producto desde dim_product (tabla en la base de datos) con el ID especificado
        cur.execute("DELETE FROM dim_product WHERE sk_product = %s", (product_id,))
        # Confirma (hace commit) en la base de datos para confirmar la eliminación.
        conn.commit()
        ...
        # Cierra el cursor y la conexión a la base de datos, independientemente de si hubo un error o no
        cur.close()
        conn.close()
